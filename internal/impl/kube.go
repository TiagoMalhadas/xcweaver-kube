...
type deployment struct {
	...
	config       *kubeConfig       // [kube] config from xcweaver.toml
	app          *protos.AppConfig // parsed xcweaver.toml
	groups       []group           // groups
}
...
func buildContainer(d deployment, g group) (corev1.Container, error) {
	...
	c := corev1.Container{
		...
		Args:            append([]string{"babysitter", "/xcweaver/xcweaver.toml", "/xcweaver/config.textpb"}, g.Components...),
		Ports:           ports,
		VolumeMounts: []corev1.VolumeMount{
			{
				Name:      "config",
				MountPath: "/xcweaver/xcweaver.toml",
				SubPath:   "xcweaver.toml",
			},
			{
				Name:      "config",
				MountPath: "/xcweaver/config.textpb",
...
func header(d deployment, filename string) (string, error) {
	...
	header := template.Must(template.New("header").Parse(`# This file was generated by "xcweaver kube" version {{.ToolVersion}} for the following
# application:
#
#     app: {{.App}}
#     version: {{.Version}}
...
// configFilename is the name of the xcweaver.toml configuration file.
func generateConfigMap(w io.Writer, configFilename string, d deployment) error {
	...
	c := corev1.ConfigMap{
		...
		Data: map[string]string{
			"xcweaver.toml": string(weaverToml),
